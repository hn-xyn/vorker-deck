...
(() => {
  ...
  class DeckStage extends HTMLElement {
    static get observedAttributes() { return ['width', 'height', 'noscale', 'no-rail', 'zoom']; }
    ...
    _fit() {
      ...
      // Optional `zoom` multiplier (0.2–1) lets the deck render smaller than
      // full-fit, with breathing room around it — useful for reviewing.
      const zAttr = parseFloat(this.getAttribute('zoom'));
      const z = (isFinite(zAttr) && zAttr > 0) ? zAttr : 1;
      this._canvas.style.transform = `scale(${fit * z})`;
    }
